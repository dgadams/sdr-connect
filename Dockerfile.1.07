...
# SDR Connect 1.0.7
ADD https://sdrplay.com/software//SDRconnect_linux-x64_22b2d4724.run  sdrc.run
RUN <<EOR
#!/bin/bash
	apt-get -yq update
    apt-get -yq install libusb-1.0-0 swig libasound2 libuuid1 libmp3lame-dev libicu76 busybox

#   install and cleanup SDRconnect
	chmod +x sdrc.run
    ./sdrc.run --tar xvf .
    rm -f *.ico *.txt *.dbg install.sh sdrc.run SDRconnect_headless
